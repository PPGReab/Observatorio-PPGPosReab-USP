...
metricas_artigos$Qualis[is.na(metricas_artigos$Qualis)] <- "NA"
...
# drop NA values
qualis.PPG <- qualis.PPG[qualis.PPG != "NA"]
...
axis(1, at = round(c(
  -max(qualis.PPG), 0, max(qualis.PPG)
)), labels = round(c(
  max(qualis.PPG), 0, max(qualis.PPG)
)))
...
summ.table.producao <-
  cbind(
    ...
    paste0(
      formatC(
        round((sum(table.2[which(rownames(table.2) != c("A1", "A2", "A3", "A4", "B1", "B2", "B3", "B4", "B5")),]) / sum(table.2) * 100), 0),
        format = "d",
        digits = 0,
        big.mark = "."
      ),
    ...
    paste0(
      formatC(
        sum(table.2[which(rownames(table.2) == c("B1", "B2", "B3", "B4")),]) / sum(table.2) * 100,
        format = "d",
        digits = 0,
        big.mark = "."
      ),
    ...
    paste0(
      formatC(
        sum(table.2[which(rownames(table.2) == c("A1", "A2", "A3", "A4")),]) / sum(table.2) * 100,
        format = "d",
        digits = 0,
        big.mark = "."
      ),
