...
plot.1 <- plot.1 %>% plotly::layout(
  ...
  xaxis = list(
    ...
    tickfont = list(color = 'white'),
    tickvals = anos,
    range = c(min(as.numeric(anos)), max(as.numeric(anos)))
  ),
  yaxis = list(
    title = "N",
    titlefont = list(color = 'white'),
...
total <- length(unique(posdoc.raw$`Identificador do Pós-Doc`))
concluidos <- sum(!is.na(posdoc.list[[as.character(max(as.numeric(names(posdoc.list))))]]$`Vínculo Fim`))
brasileiros.concluidos <- sum(
