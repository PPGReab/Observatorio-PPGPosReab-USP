...
if (has.sucupira.files) {
  label.quad <-
    paste0("Período ", as.character(min(anos)), "-", as.character(max(anos)))
} else {
  label.quad <- ""
}
text(
...
# save PDF
grDevices::pdf(file = "PPG/Images/Cover-3.pdf",
               width = W,
               height = H)
img <- png::readPNG("PPG/Images/Cover-3.png")
