get_id_metadata <- function (id, subtipo = NULL, produtos.detalhes) {
  # initialize detalhamento
  detalhamento <- ""
  
  # check input
  if (is.null(produtos.detalhes)) {
    return(detalhamento)
  } else {
    # select products
  ...
  if (subtipo == "ARTIGO EM PERIÓDICO") {
    ...
    detalhamento <-
      paste0(
        ...
        if (!sjmisc::is_empty(Volume))
          paste0(", V. ", Volume),
        if (!sjmisc::is_empty(Fascículo))
          paste0(", N. ", Fascículo),
        if (!sjmisc::is_empty(Série))
          paste0(", Série ", Série),
        if (!sjmisc::is_empty(Inicial) &
            sjmisc::is_empty(Final))
          paste0(", p. ", Inicial),
        if (!sjmisc::is_empty(Final))
          paste0("-", Final),
        if (!sjmisc::is_empty(DOI))
          paste0(
            ", DOI: ",
            " <a href='",
            "https://doi.org/",
  ...
  if (subtipo == "EDITORIA") {
    ...
    detalhamento <-
      paste0(
        paste0("<b>", Natureza, "</b>"),
        if (!sjmisc::is_empty(Promotora))
          paste0(", ", Promotora),
        if (!sjmisc::is_empty(Editora))
          paste0(", ", Editora),
        if (!sjmisc::is_empty(Tipo))
          paste0(", ", Tipo),
        if (!sjmisc::is_empty(Páginas))
          paste0(", ", Páginas, " p."),
        if (!sjmisc::is_empty(DOI))
          paste0(
            ", DOI: ",
  ...
  if (subtipo == "LIVRO") {
    ...
    detalhamento <-
      paste0(
        paste0("<b>", Obra, "</b>"),
        if (!sjmisc::is_empty(Páginas.Obra))
          paste0(", ", Páginas.Obra, " p."),
        if (!sjmisc::is_empty(Contribuição))
          paste0(", ", Contribuição),
        if (!sjmisc::is_empty(Páginas.Contribuição))
          paste0(", ", Páginas.Contribuição, " p."),
        if (!sjmisc::is_empty(Editora))
          paste0(", ", Editora),
        if (!sjmisc::is_empty(ISBN))
          paste0(", ISBN: ", ISBN),
        if (!sjmisc::is_empty(URL))
  ...
  if (subtipo == "TRABALHO EM ANAIS") {
    ...
    detalhamento <-
      paste0(
        paste0("<b>", Evento, "</b>"),
        if (!sjmisc::is_empty(Título))
          paste0(", ", Título),
        if (!sjmisc::is_empty(País))
          paste0(", ", País),
        if (!sjmisc::is_empty(Volume))
          paste0(", ", Volume),
        if (!sjmisc::is_empty(Fascículo))
