...
for(i in 1:length(turmas.ID)) {
  ...
  table.turmas$Ano[i] <- as.numeric(unlist(strsplit(unique(turma$`Período/Ano`), "/"))[2])
  table.turmas$Período[i] <- as.numeric(unlist(strsplit(unique(turma$`Período/Ano`), "/"))[1])
  ...
  # get turma responsável
  table.turmas$`Responsável`[i] <- paste0(
    stringr::str_to_title(unique(turma$`Nome do responsável`[turma$`Indicador de responsável principal` == "Sim"])),
    " (",
    unique(turma$`Categoria do responsável`[turma$`Indicador de responsável principal` == "Sim"]),
    ")"
  )
  # get turma colaboradores if existent
  if(sum(turma$`Indicador de responsável principal` == "Não") > 0){
    table.turmas$`Colaboradores`[i] <- paste0(
      stringr::str_to_title(turma$`Nome do responsável`[turma$`Indicador de responsável principal` == "Não"]),
      " (",
      turma$`Categoria do responsável`[turma$`Indicador de responsável principal` == "Não"],
      ")",
      collapse = ", "
    )
  }
...
table.turmas <- table.turmas[order(-table.turmas$Ano, -table.turmas$Período, table.turmas$Disciplina, table.turmas$Nível),]
