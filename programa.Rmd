...
for (i in 1:dim(area.capes)[1]) {
  if (!is.na(area.capes[i, ncol(area.capes)])) {
    Abrir[i] <-
      paste0('<a href="',
             area.capes[i, ncol(area.capes)],
             '" target="_blank"',
             '>',
             # fontawesome icon
...
area.capes <- cbind(area.capes[,-ncol(area.capes)], Abrir)
...
Abrir <- matrix(NA, nrow = dim(agendas)[1])
colnames(Abrir) <- "Abrir"
# add hyperlinks
for (i in 1:dim(agendas)[1]) {
  if (!is.na(agendas[i, ncol(agendas)])) {
    Abrir[i] <-
      paste0('<a href="',
             agendas[i, ncol(agendas)],
             '" target="_blank"',
             '>',
             # fontawesome icon
             fontawesome::fa("up-right-from-square"),
...
agendas <- cbind(agendas[,-ncol(agendas)], Abrir)

# print tables
source("Scripts/table-with-buttons.R", local = knitr::knit_global())
create_dt(agendas, title = "Programa - Área CAPES")
...
## **Bolsas de estudo** {#bolsas-de-estudo .tabset}
