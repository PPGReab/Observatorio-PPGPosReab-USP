...
# replace NA values with (Sem Área Associada) in area de concentração
table.projetos$`Área de Concentração`[is.na(table.projetos$`Área de Concentração`)] <- "Não Vinculada"

# replace NA values with (Sem Linha Associada) in linhas de pesquisa
table.projetos$`Linha de Pesquisa`[is.na(table.projetos$`Linha de Pesquisa`)] <- "Não Vinculada"

# get unique linhas de pesquisa
linhas <- unique(table.projetos$`Linha de Pesquisa`)

# loop to show separate tables by Linha de Pesquisa
for (i in 1:length(linhas)) {
  cat(knitr::knit_print(
    knitr::kable(
      x = table.projetos[table.projetos$`Linha de Pesquisa` == linhas[i], ],
      align = "l",
      escape = FALSE,
      format = ifelse(knitr::is_html_output(), "html", "latex"),
      booktabs = TRUE,
