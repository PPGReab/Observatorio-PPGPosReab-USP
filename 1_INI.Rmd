...
doi_artigos_completos_sucupira <-
  data.frame(DOI = matrix(NA, nrow = 0, ncol = 1))
if (dim(doi_artigos_completos_sucupira)[1] != 0) {
  # data filter
  for (i in 1:length(titles)) {
    doi_artigos_completos_sucupira$DOI <- rbind(
      doi_artigos_completos_sucupira$DOI,
  ...
  doi_artigos_completos_sucupira <- unique(doi_artigos_completos_sucupira)
...
if (dim(doi_artigos_resumos_sucupira)[1] != 0) {
  # data filter
  for (i in 1:length(titles)) {
    doi_artigos_resumos_sucupira$DOI <- c(
      doi_artigos_resumos_sucupira$DOI,
      sucupira.raw$`Valor do Item de Detalhamento`[which(
        and(
          sucupira.raw$`Nome da Produção` == titles[i],
          sucupira.raw$`Item de Detalhamento` == item.detalhamento
  ...
  doi_artigos_resumos_sucupira <- unique(doi_artigos_resumos_sucupira)
...
if (dim(doi_anais_sucupira)[1] != 0) {
  ...
  doi_anais_sucupira <- unique(doi_anais_sucupira)
...
if (dim(doi_livros_sucupira)[1] != 0) {
  ...
  doi_livros_sucupira <- unique(doi_livros_sucupira)
