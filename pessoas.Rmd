...
if (dim(docentes)[1] != 0) {
  ...
  table.2 <- table.2[order(table.2$'Nome', decreasing = FALSE), ]
}
...
# drop columns
discentes <- discentes %>% dplyr::select("Nível Discente", "Ano de Matrícula", "Data Matrícula", "Nome Discente", "Nome do Orientador", "Tempo em curso", "Defesa até")
...
if (dim(discentes)[1] != 0) {
  ids <- order(discentes$'Ano de Matrícula', discentes$'Nome Discente', decreasing = TRUE)
  discentes <- discentes[ids,]
}
source("Scripts/table-with-buttons.R", local = knitr::knit_global())
...
bancas <- data.frame(
  ...
  "Internacionalização" = rep(NA, length(id.TCC)),
  check.names = FALSE
)
# loop over each ID
for (i in 1:length(id.TCC)) {
  ...
  bancas$Internacionalização[i] <- ifelse(sum(unique(banca$"País da Pessoa da Banca") != "Brasil", na.rm = TRUE) > 0, paste0(unique(banca$"País da Pessoa da Banca")[banca$"País da Pessoa da Banca" != "Brasil"], collapse = ", "), "Não")
}
