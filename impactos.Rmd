...
```{r politicas-sintese, eval = has.sage.files}
...
# drop columns
produtos <- produtos %>% dplyr::select("ID da Produção", "Ano da Produção", "Subtipo de Produção", "Nome da Produção", "Nome do Autor", "Número de Ordem do Autor", "Categoria do Autor")

# format names
produtos$`Subtipo de Produção` <- tools::toTitleCase(tolower(produtos$`Subtipo de Produção`))
...
if (dim(produtos)[1] != 0) {
  for (i in 1:length(unique(produtos$"ID da Produção"))) {
    ...
    metadata <- get_id_metadata(id = id, subtipo = produtos$`Subtipo de Produção`[i], produtos.detalhes = produtos.detalhes)
    produtos.list[[i]] <- data.frame(
      "Ano" = produtos.id$"Ano da Produção"[1],
      "Tipo" = produtos.id$"Subtipo de Produção"[1],
...
### **Produção técnica** {#producao-bibliografica}
