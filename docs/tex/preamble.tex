% Load packages
\usepackage[brazilian]{babel}
\usepackage{fontspec}
\usepackage{lmodern} % Use Latin Modern font
\usepackage{amsmath} % For math functions
\usepackage{academicons}
\usepackage{fontawesome5}

% from kable
\usepackage{booktabs}
\usepackage{longtable}
\usepackage{array}
\usepackage{multirow}
\usepackage{multicol}
\usepackage{wrapfig}
\usepackage{float}
\usepackage{placeins}
\usepackage{colortbl}
\usepackage{pdflscape}

% patch for tabu
\makeatletter
\def\tabu@verticalspacing{\ifnum\everyeof{\noexpand}\the\everyeof\space=%
  \relax \@tempdima\dimexpr\ht\tabu@arstrutbox+\dp\tabu@arstrutbox\relax
  \else \@tempdima\z@ \fi
  \setbox\tabu@arstrutbox\hbox{\vrule
    \@height\ht\tabu@arstrutbox \@depth\dp\tabu@arstrutbox \@width\z@}
  \nointerlineskip \vskip\@tempdima}
\makeatother
\usepackage{tabu}

\usepackage{threeparttable}
\usepackage{threeparttablex}
\usepackage[normalem]{ulem}
\usepackage{makecell}
\usepackage{xcolor}
\usepackage{tabularray}
\usepackage{ltablex}
\usepackage{tabularx}
\keepXColumns
\usepackage{graphicx}
\usepackage{letltxmacro}
\usepackage{pdfpages}
\usepackage[notransparent]{svg}
\usepackage{tcolorbox}
\usepackage{titlesec}
\usepackage{tocloft}
\usepackage[linktoc=all]{hyperref}

% Define color
\definecolor{lightgray}{gray}{0.95}

% Define tcolorbox
\newtcolorbox{blackbox}{
  colback=lightgray,
  colframe=orange,
  coltext=black,
  boxsep=5pt,
  arc=4pt
}

% Define landscape environment to use with large PDF tables
\usepackage{pdflscape}
\newcommand{\blandscape}{\begin{landscape}}
\newcommand{\elandscape}{\end{landscape}}

% Define infobox environment
\newenvironment{infobox}[1]
  {
  \begin{itemize}
  \renewcommand{\labelitemi}{\raisebox{-.7\height}[0pt][0pt]{\includegraphics[width=3em,keepaspectratio]{#1}}}
  \setlength{\fboxsep}{1em}
  \begin{blackbox}
  \item
  }
  {
  \end{blackbox}
  \end{itemize}
  }

% Enable/disable footnotes
\LetLtxMacro\Oldfootnote\footnote
\newcommand{\EnableFootNotes}{\LetLtxMacro\footnote\Oldfootnote}
\newcommand{\DisableFootNotes}{\renewcommand{\footnote}[2][]{\relax}}

% to Portuguese in long tables
\DefTblrTemplate{contfoot-text}{default}{Continuado na próxima página}
\DefTblrTemplate{conthead-text}{default}{(Continuado)}

% Watermark
% \usepackage{draftwatermark}
% \SetWatermarkLightness{0.9}
% \SetWatermarkText{RASCUNHO}
% \SetWatermarkScale{1}

% Remove the title page
\AtBeginDocument{\let\maketitle\relax}

% Arabic page numbering for backmatter
\pagenumbering{arabic}
